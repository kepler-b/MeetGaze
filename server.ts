...
import { bgRed, red, yellow, bgGreenBright, greenBright } from 'colorette';
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("joined-ice-candidate", ({ roomId, icecandidate }): void => {
    if (!adminAndRoomID.has(roomId)) {
      printError("On-join-ice-candidate: can't find admin id with roomid or admin doesn't exist");
      return;
    }
